...
export default function HistoryPage() {
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-5 py-8">
      <h1 className="font-serif text-2xl font-bold text-brand-dark">My Bills</h1>

      <div className="mt-5 grid items-start gap-6 lg:grid-cols-[1fr_1.1fr]">
        <div className="space-y-3">
          {bills.map(b => (
            <div key={b.id} className="rounded-card border border-line bg-card p-4 transition hover:border-brand hover:shadow-panel">
              <div className="flex items-center justify-between gap-3">
                <Link to={`/bills/${b.id}`} className="min-w-0 flex-1">
                  <div className="text-base font-semibold text-brand-dark">{b.billingMonth}</div>
                ...
                <div className="text-right">
                  <div className="text-base font-bold">{rs(b.payableWithinDue ?? b.currentBill)}</div>
                  <Link to={`/bills/${b.id}`} className="text-sm text-brand hover:underline">see breakdown →</Link>
                </div>
              </div>
              ...
              <div className="mt-2.5 border-t border-line pt-2.5">
                {confirming === b.id ? (
                  ...
                  <button onClick={() => setConfirming(b.id)} className="text-sm text-ink-muted transition hover:text-danger">
                    🗑 Delete bill
                  </button>
                )}
              </div>
        ...
        {chartData.length > 1 && (
          <div className="rounded-panel border border-line bg-card p-5 shadow-panel lg:sticky lg:top-24">
            <h2 className="text-sm font-semibold text-ink-muted">Units used per month</h2>
