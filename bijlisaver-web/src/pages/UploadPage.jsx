...
function SampleReport() {
  ...
  return (
    <div className="relative">
      ...
      <div className="rounded-panel border-2 border-brand/25 bg-card p-6 pt-8 shadow-panel">
        <div className="flex items-baseline justify-between border-b border-dashed border-line pb-3">
          <b className="text-base text-brand-dark">June Bill — what you'll get</b>
          <span className="text-base text-ink-muted">312 units</span>
        </div>
        <div className="mt-3 space-y-1">
          {DEMO_ROWS.map((r, idx) => (
            <div
              ...
              className={`cursor-default rounded-lg px-2.5 py-2.5 transition ${active === idx ? 'bg-brand-tint' : ''}`}
            >
              <div className="flex items-baseline justify-between text-base">
                <span className="font-medium">{r.label}</span>
                <span className="font-semibold">{r.amount}</span>
              </div>
              <p className={`mt-1 text-sm leading-relaxed transition ${active === idx ? 'text-ink' : 'text-ink-muted'}`}>
                {r.note}
              </p>
            </div>
          ))}
        </div>
        <div className="mt-3 rounded-r-lg border-l-[3px] border-warn bg-warn-tint px-3 py-2.5 text-base font-semibold text-warn-ink">
          23% of this bill is taxes and extra charges — not electricity.
        </div>
        <div className="mt-3 flex items-baseline justify-between border-t-2 border-ink px-2.5 pt-3 text-lg font-bold">
          <span>Total Payable</span><span>Rs 8,540</span>
        </div>
        <div className="mt-3 rounded-card bg-brand-tint p-3.5">
          <div className="text-sm font-semibold text-brand-dark">💰 Your savings plan</div>
  ...
  return (
    <>
      ...
      <section className="hero-grid">
        <div className="mx-auto grid w-full max-w-6xl items-center gap-14 px-5 pb-20 pt-16 lg:grid-cols-[1.05fr_0.95fr]">
          <div>
            <span className="fade-up inline-flex items-center gap-2 rounded-full bg-brand-tint px-3.5 py-2 text-sm font-semibold uppercase tracking-wider text-brand">
              <span className="pulse-dot inline-block h-1.5 w-1.5 rounded-full bg-brand" />
              For LESCO bills · Lahore
            </span>
            <h1 className="fade-up delay-1 mt-6 font-serif text-4xl font-bold leading-[1.18] text-brand-dark lg:text-5xl">
            ...
            <p dir="rtl" className="fade-up delay-2 mt-5 text-left font-urdu text-2xl text-ink-muted">
              بل کی تصویر بھیجیں — ہم ہر روپیہ سمجھا دیں گے
            </p>
            <p className="fade-up delay-2 mt-4 max-w-[48ch] text-base leading-relaxed text-ink-muted">
            ...
            <div className="fade-up delay-3 mt-7 flex flex-wrap gap-x-8 gap-y-2 text-base text-ink-muted">
      ...
      <section className="border-y border-line bg-card">
        <div className="mx-auto w-full max-w-3xl px-5 py-16 text-center">
          <h2 className="font-serif text-3xl font-bold text-brand-dark">Try it with your bill</h2>
          <p className="mt-2 text-base text-ink-muted">One photo. Thirty seconds. Every rupee explained.</p>

          <div
            id="upload-zone"
            className={`mx-auto mt-8 cursor-pointer rounded-panel border-2 border-dashed p-12 transition
          ...
          >
            ...
            {file ? (
              <p className="mt-4 text-lg font-semibold text-brand-dark">{file.name}</p>
            ) : (
              <>
                <p className="mt-4 text-lg font-semibold text-brand-dark">Tap to choose your bill photo</p>
                <p className="mt-2 text-base text-ink-muted">or drag and drop · JPG, PNG or PDF · even a blurry phone photo works</p>
              </>
            )}
          </div>
          ...
          <button
            onClick={submit}
            disabled={!file || busy}
            className="mt-6 w-full max-w-md rounded-btn bg-brand px-6 py-4 text-lg font-semibold text-white transition
      ...
      {/* ============ STAT STRIP ============ */}
      <section className="mx-auto grid w-full max-w-6xl gap-8 px-5 py-14 text-center sm:grid-cols-4">
        {[
          ...
          <div key={label}>
            <div className="font-serif text-4xl font-bold text-brand">{num}</div>
            <div className="mt-1.5 text-sm font-medium uppercase tracking-wider text-ink-muted">{label}</div>
          </div>
        ))}
      </section>
      ...
      <section className="border-y border-line bg-card">
        <div className="mx-auto w-full max-w-6xl px-5 py-16">
          <h2 className="text-center font-serif text-3xl font-bold text-brand-dark">Three simple steps</h2>
          <p className="mt-2 text-center text-base text-ink-muted">No sign-up. No payment. No confusing words.</p>
          <div className="mt-10 grid gap-6 md:grid-cols-3">
            {[
              ...
              <div key={title} className="rounded-card border border-line bg-paper p-7 transition hover:-translate-y-1 hover:shadow-panel">
                <div className="grid h-12 w-12 place-items-center rounded-chip bg-brand-tint text-2xl">{icon}</div>
      ...
      <section className="mx-auto w-full max-w-6xl px-5 py-16">
        <div className="grid items-center gap-10 rounded-panel bg-[#0E3B2E] p-10 text-night-text lg:grid-cols-2">
          <div>
            <h2 className="font-serif text-3xl font-bold">The 200-unit trap, explained</h2>
          ...
          <div className="rounded-card bg-[#0A2C22] p-6">
            <div className="text-base font-semibold">Example: 312 of 400 units used</div>
            ...
            <div className="mt-4 text-base font-semibold text-warn">
